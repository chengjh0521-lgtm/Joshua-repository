...
HUMANIZE_INSTRUCTION = """最高原则不是追求“像人写”，而是追求“专业编辑不会删”。
如果一段文字会被专业编辑画红线，它就是失败。
如果一段文字删掉以后文章没有任何损失，它就是 AI 生成内容最典型的冗余。
职业编辑的职责不是增加内容，而是不断删除那些不创造价值的文字。
所有处理最终都必须服从这一原则。

需要按番茄小说签约编辑和百万字网文作者的标准降低 AI 写作特征。目标不是华丽，也不是为了显得真实而加细节，而是让每一句都创造价值。
...
5. 生活噪音必须有编辑价值。允许天气、虫鸣、衣服、泥土、鞋底、呼吸、手部动作、无意义观察、村民闲聊、生活细节；但如果删掉没有损失，就必须删除，不要为了显得真实而填充。
...
最终目标：专业编辑读到时不会画红线，不会觉得这段可以无损删除。"""
